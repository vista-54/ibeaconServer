...
namespace common\components\cldwdg;

use common\components\cldwdg\CalendarViewAsset;
use common\components\cldwdg\CalendarViewDateTime;
 ...
 *
 *         'views'     => [
 *             'calendar' => '@vendor/marekpetras/cldwdg/views/calendar',
 *             'month' => '@vendor/marekpetras/cldwdg/views/month',
 *             'day' => '@vendor/marekpetras/cldwdg/views/day',
 *         ],
 *         'startYear' => date('Y') - 1,
 *         'endYear' => date('Y') + 1,
 *
...
//        setSort(['attributes' => [$this->dateField]]);
//        $modelsArr=array();
//

//            var_dump($this->dataProvider->getModels());
        foreach ($this->dataProvider->getModels() as $model) {
//            var_dump(Yii::$app->request->get('id'));
            /*Check The Event ID*/
//            var_dump(Yii::$app->request->get('location_id'));
            if ($model->location_id == Yii::$app->request->get('id')) {
//                    var_dump($model);
//                array_push($modelsArr,$model);

                $this->models[date($this->calendarFormat, strtotime($model->{$this->dateField}))][] = $model;
//                var_dump($model->{$this->dateField});
            }
        }
...
//                    var_dump($models[1]->{$this->dateField});
                    $link = null;
